...
public class RenderInstancesProceduralPass : ScriptableRenderPass
{
    ...
    void UpdateFunctionOnGPU(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        using (new ProfilingScope(commandBuffer, m_ProfilingSampler))
        {
            ...
            //commandBuffer.DrawProcedural(Matrix4x4.identity, instanceMaterial, 0, MeshTopology.Quads, ((int)resolution) * ((int)resolution), 1, m_MatBlock);

            commandBuffer.DrawProceduralIndirect(Matrix4x4.identity, instanceMaterial, 0, MeshTopology.Lines, bufferWithArgs, 0, m_MatBlock);//Quad会报错
        }
